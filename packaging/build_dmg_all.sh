#!/bin/bash
# 從頭到尾一次把 11+ 跟 10.15 兩包都打出來：swift build -> 組裝 .app ->
...
# build_dmg_11_plus.sh / build_dmg_10_15.sh 即可，那兩支維持原本
# 「不自動重新 build」的行為不變。
set -euo pipefail
...
echo "==> [1/4] swift build + 組裝 .app（11+）"
bash "$REPO_ROOT/packaging/build_app_11_plus.sh" "$CONFIG"

echo
echo "==> [2/4] swift build + 組裝 .app（10.15）"
bash "$REPO_ROOT/packaging/build_app_10_15.sh" "$CONFIG"

echo
echo "==> [3/4] 封裝 DMG（11+）"
bash "$REPO_ROOT/packaging/build_dmg_11_plus.sh" "$CONFIG"

echo
echo "==> [4/4] 封裝 DMG（10.15）"
bash "$REPO_ROOT/packaging/build_dmg_10_15.sh" "$CONFIG"
