...
// macOS 12 以下、只留 Stage 2 的那包（見 zh-cn-to-tw-mac/README 的
// 「版本分流」說明）。跟根目錄的 Package.swift（13+，platforms .v13）
// 刻意是兩個獨立的 SwiftPM package，不是同一個 package 裡的兩個
// target——SwiftPM 的 platforms 是整個 package 共用一份，沒有 per-target
// 的部署目標，沒辦法在同一個 Package.swift 裡讓兩個 target 各自要求
// 不同的最低系統版本。
//
...
let package = Package(
    ...
    targets: [
        .executableTarget(
            ...
            // 藉此決定要不要把 Stage 1 鎖住、要送哪個 os_version 給
            // version_check API——見 ContentView.swift 的 osTier 說明。
            // 根目錄那個 13+ target 沒有定義這個 flag，同一份原始碼
            // 在那邊會走 #else 分支。
